...
cProdukt::cProdukt(float aCena, string aNazwa, int aLp) :cena(aCena), nazwa(aNazwa), lp(aLp){
}
...
cProdukt* cProdukt::cKlonuj(){
    return new cProdukt(this->cena, this->nazwa, this->lp);
}
